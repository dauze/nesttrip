...
export class FlightStatusBadgeComponent {
  ...
  readonly logistic = input.required<FlightLogistic>();

  readonly meta = computed(() => badgeMetaFor(this.logistic()));
  ...
  constructor() {
    ...
    effect((onCleanup) => {
      ...
      const phase = getRefreshPhase(logistic, new Date());
      const intervalMs = REFRESH_INTERVAL_MS[phase];

      this.refreshService.refreshIfStale(tripId, logistic);
      if (intervalMs === null) return;

      const id = setInterval(() => this.refreshService.refreshIfStale(tripId, logistic), intervalMs);
  ...
  private manualRefresh(): void {
    ...
    this.refreshService.forceRefresh(this.tripId(), this.logistic()).subscribe({
