...
import { Reservation, ReservationFile } from '@core/models/reservation.dto';
 ...
 * directement `reservation().id`.
 */
@Component({
  selector: 'app-reservation-files',
  standalone: true,
  imports: [ChipComponent, ButtonComponent, ProgressSpinnerComponent],
  templateUrl: './reservation-files.component.html',
  styleUrl: './reservation-files.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class ReservationFilesComponent {
  ...
  onFileSelect(event: Event): void {
    ...
    for (const file of files) {
      const path = `trips/${this.tripId()}/reservations/${reservation.id}/${file.name}`;
      this.uploadingFiles.update((s) => new Set(s).add(file.name));

      this.fileService.uploadFile(file, path).pipe(
        tap(({ url, name }) => {
          this.tripFacade.updateReservation(this.tripId(), {
            ...reservation,
            files: [...(reservation.files ?? []), { name, url, path }],
          });
        }),
      ).subscribe({
  ...
  removeFile(index: number): void {
    const reservation = this.reservation();
    const file = reservation.files[index];
    this.fileService.deleteFile(file.path).pipe(
      tap(() => {
        this.tripFacade.updateReservation(this.tripId(), {
          ...reservation,
          files: reservation.files.filter((_, i) => i !== index),
        });
      }),
    ).subscribe();
  }

  openFile(file: ReservationFile): void {
