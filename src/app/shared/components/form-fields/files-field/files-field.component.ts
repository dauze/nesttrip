...
import { FileRepository } from '@core/infra/firebase/services/file-repository';
...
export class FilesFieldComponent {
  ...
  onFileSelect(event: Event): void {
    ...
    for (const file of selected) {
      ...
      this.fileRepository.uploadFile(file, path).pipe(
        tap(({ url, name }) => {
  ...
  removeFile(index: number): void {
    ...
    this.fileRepository.deleteFile(file.path).pipe(
      tap(() => {
