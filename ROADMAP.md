...
- Génération de voyage assistée par IA (voir "Nouveau voyage / IA" ci-dessous et `src/specs/process-creation-trip-ia.md`) : repris malgré le statut "non prioritaire", à la demande explicite de l'utilisateur. Découpage en lots suivi tel que défini dans la spec (§7) :
  ...
  - **Réserves communes aux Lots 2/3** :
    - Aucune clé Anthropic/LLM fournie — la sélection reste un stub déterministe (`select-activities-stub.ts`), pas une vraie IA. Isolé derrière cette seule fonction pour brancher le vrai appel plus tard sans toucher le reste du pipeline.
    - Aucune vérification visuelle réelle possible cette session (pas de compte de test `.env.e2e`, pas de projet Firebase déployé pour tester les Cloud Functions en conditions réelles) — seuls lint/typecheck/tests unitaires et `tsc`/build des Cloud Functions ont pu être vérifiés.
