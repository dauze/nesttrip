...
- Améliorer le sous-menu "Logistique" pour qu'il fasse plus "fondu" (accordéon groupé par type ?) — volontairement exclu du lot de refonte Logistique (voir "✅ Déjà fait") : décision produit prise avec l'utilisateur de ne pas le traiter dans ce lot, reste à cadrer (comportement précis de repli/groupement) avant de le reprendre.
...
- Trouver un moyen élégant de ne pas mettre le titre en premiere zone de saisie pour l'ordi, on croit qu'il faut le renseigner alors que non pour les train et vol. Pour logement, mettre un placeholder google et faire la même cinématique pour afficher ou non l'adresse si il saisie un placeid ou pas. Pour autre, rien ne change  
- si on clique sur + depuis un certain jour, il faut renseigner la date de début avec le jour sur lequel on a cliqué. 
- La date de début d'une réservation doit être positionné soit sdur le jour cliqué si on vient du jouer, soit la date de début du voyage ! 
- La date de fin d'un réservation doit être alimentée de la date de dévbut une fois qu'elle est sélectionnée
- L'heure de fin doit être alimentée de l'heure de début lorsque l'on saisie une réservation ou une activité 
### Bugs / fixes
...
- Le calendrier n'aparait plus en vue ordinateur à tor, on doit pôuvoir modifier la date via la saisie ET via le calendrier
...
- Si on choisi un prix à 0, il faut afficher 0 pas "Ajouter un prix"
