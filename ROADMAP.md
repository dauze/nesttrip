...
- Dialogue note : le repositionnement clavier (VisualViewportService) ne fonctionne pas sur cette popup mais sur tous les autres oui, lorsque j'affiche la popup, le clavier s'ouvre, et la popup n'est pas décalée vers le haut comme pour les autres popup. **Restant après la passe du 2026-08-13** : limite 5000 + animation du compteur faites (voir ROADMAP_already done.md) — seul ce point clavier reste. Investigation code faite (mécanisme CSS `--nt-visual-viewport-height`/`panelClass` identique à `TitleEditDialogComponent`, qui fonctionnerait correctement d'après le retour utilisateur) sans trouver de différence structurelle expliquant le bug : nécessite une reproduction sur un vrai device (pas de compte de test `.env.e2e` ni de clavier virtuel simulable dans ce bac à sable) avant de tenter un correctif.
...
- Dans les settings du trip, pouvoir changer la destination PRINCIPALE (`Trip.ville`/`placeId`) — pas encore possible aujourd'hui (aucun champ éditable pour ça dans `TripSettingsSectionComponent`), et cela changerait aussi la photo. **Restant après la passe du 2026-08-13** : la partie "destinations additionnelles" (multi-destination) est faite — vrai champ `Trip.additionalCities`, éditable création (avant le choix manuel/IA) et settings, voir `ROADMAP_already done.md`. Seul le changement de la destination PRINCIPALE + rafraîchissement de la photo reste à faire — décision explicitement pas prise seul (nécessite de choisir l'UX : un même dialog que les destinations additionnelles ? un champ séparé ? remplacement direct ou permuter avec une destination additionnelle existante ?). 
