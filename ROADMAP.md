...
- Génération de voyage assistée par IA (voir "Nouveau voyage / IA" ci-dessous et `src/specs/process-creation-trip-ia.md`) : repris malgré le statut "non prioritaire", à la demande explicite de l'utilisateur. Découpage en lots suivi tel que défini dans la spec (§7) :
  ...
  - **Lot 2 (pipeline `activities_only` end-to-end) — fait, avec 2 réserves à lever avant mise en prod.** Nouvelle collection Firestore séparée `tripGenerations/{tripId}` (délibérément hors du doc `trips/{tripId}` — voir `TripGeneration`, doc de classe) + Cloud Function déclenchée par `onDocumentWritten` (`functions/src/trip-generation/generate-trip.trigger.ts`, couvre création ET "Régénérer tout") : recherche Google Places élargie par centre d'intérêt (`searchNearby`, `search-activities.ts`) → sélection (`select-activities-stub.ts`, heuristique round-robin par intérêt/note — **PAS un appel LLM réel**, aucune clé fournie dans cette session, isolé derrière une seule fonction pour un branchement futur sans toucher le reste du pipeline) → écran de génération (`/trips/:id/generating`) → aperçu avec Exclure/Remplacer/Régénérer (`/trips/:id/preview`) → "Valider" crée les vraies activités (`PoolActivity.source: 'ai_generated'`, badge "Suggéré par IA" sur la carte). `activities_day`/`full_plan` restent bloqués côté formulaire (message explicite) tant que non implémentés.
    - **Réserve 1** : aucune clé Anthropic/LLM fournie — la sélection actuelle est un stub déterministe, pas une vraie IA. À brancher dans `select-activities-stub.ts` (remplacer cette seule fonction) une fois une clé disponible.
    - **Réserve 2** : aucune vérification visuelle réelle possible cette session (pas de compte de test `.env.e2e`, pas de projet Firebase déployé pour tester la Cloud Function en conditions réelles) — seuls lint/typecheck/tests unitaires et `tsc`/build des Cloud Functions ont pu être vérifiés.
